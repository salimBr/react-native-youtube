...
- (instancetype) initWithVideoIdentifier:(NSDictionary *)videoInfos
{
	...
	if (videoInfos)
//        self.videoIdentifier = [videoInfos  objectForKey:@"videoId"];
    self.videoIdentifier = videoInfos;
...
- (void) setVideoIdentifier:(NSDictionary *)videoInfos
{
    
    NSLog(@"video infos ==== %@",videoInfos);
    
//    if ([videoIdentifier isEqual:self.videoIdentifier])
//        return;
	
	_videoIdentifier = [[videoInfos objectForKey:@"videoId"] copy];
    NSDictionary * token = [videoInfos objectForKey:@"token"];
	
	[self.videoOperation cancel];
	self.videoOperation = [[XCDYouTubeClient defaultClient] getVideoWithIdentifier:_videoIdentifier token:token completionHandler:^(XCDYouTubeVideo *video, NSError *error)
	{
		if (video)
		{
			NSURL *streamURL = nil;
