...
- (instancetype) initWithVideoIdentifier:(NSString *)videoIdentifier languageIdentifier:(NSString *)languageIdentifier token:(NSDictionary *) token cookies:(NSArray<NSHTTPCookie *> *)cookies
{
	if (!(self = [super init]))
    ...
    if(token && [token objectForKey:@"access_token"]){
        NSLog(@"tokeeeeeeen ==== %@",[token objectForKey:@"access_token"]);
        sessionConfiguration.HTTPAdditionalHeaders = @{@"Authorization": [NSString stringWithFormat:@"Bearer %@",[token objectForKey:@"access_token"]]};
    }
...
{
    return [self initWithVideoIdentifier:videoIdentifier languageIdentifier:languageIdentifier token:nil cookies:nil];
}
